...
// Define the area of interest (Jodhpur district)
var jodhpur = ee.FeatureCollection('FAO/GAUL/2015/level2')
  .filter(ee.Filter.eq('ADM2_NAME', 'Jodhpur'))
  .filter(ee.Filter.eq('ADM1_NAME', 'Rajasthan')); // Adjust state name if needed
...
// Clip the mask to the Jodhpur district
var agriculturalLand = agricultureMask.updateMask(agricultureMask).clip(jodhpur.geometry());
...
function calculateIndices(image) 
{
  ...
  // Clip only once to Jodhpur district
  //return allIndices.clip(jodhpur.geometry());
  return allIndices.updateMask(agriculturalLand).clip(jodhpur.geometry());
...
years.forEach(function(year) {
  var collection = getYearlyCollection(year, jodhpur);
  print('Jodhpur Image Count:', getYearlyCollection(year, jodhpur).size());
  
  // Indices to extract
  var indices = ['ARVI','EVI','MNDVI','MNDWI','NDMI','NDVI','NDWI','NMDI','RDI','RECI','SAVI', 'TVI'];

  var timeSeries = extractTimeSeries(collection, indices, jodhpur, year);
  ...
  Export.table.toDrive({
    ...
    folder: 'L1C',
    fileNamePrefix: 'TimeSeries_Jodhpur_' + year,
    fileFormat: 'CSV'
  });
});
