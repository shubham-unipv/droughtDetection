...
// Define the area of interest (amravati district)
var amravati = ee.FeatureCollection('FAO/GAUL/2015/level2')
  .filter(ee.Filter.eq('ADM2_NAME', 'Amravati'))
  .filter(ee.Filter.eq('ADM1_NAME', 'Maharashtra')); // Adjust state name if needed
...
// Clip the mask to the Amravati district
var agriculturalLand = agricultureMask.updateMask(agricultureMask).clip(amravati.geometry());
...
// Currently the data is from Level 1C.
// And using this we have used 20 percent cloud
// pixel percentage.
// In order to use Level 2A, we can use this.
// return ee.ImageCollection('COPERNICUS/S2_SR')
// We have to also use a different cloud masking
// Technique. 
// It is done using the Scene Classificaiton Layer
function maskCloudsL2A(image) 
{
  var scl = image.select('SCL');
  // Mask clouds and cloud shadows
  var cloudMask = scl.neq(3) // Not dark area pixels
    .and(scl.neq(9))  // Not medium probability clouds
    .and(scl.neq(10)) // Not high probability clouds
    .and(scl.neq(11)) // Not thin cirrus
    .and(scl.neq(4)); // Not cloud shadows
...
function calculateIndices(image) 
{
  ...
  // Clip only once to amravati district
  //return allIndices.clip(amravati.geometry());
  return allIndices.updateMask(agriculturalLand).clip(amravati.geometry());
}
...
years.forEach(function(year) {
  var collection = getYearlyCollection(year, amravati);
  print('Thanjavur Image Count:', getYearlyCollection(year, amravati).size());
  
  // Indices to extract
  var indices = ['ARVI','EVI','MNDVI','MNDWI','NDMI','NDVI','NDWI','NMDI','RDI','RECI','SAVI', 'TVI'];

  var timeSeries = extractTimeSeries(collection, indices, amravati, year);
  ...
  Export.table.toDrive({
    ...
    folder: 'L1C',
    fileNamePrefix: 'TimeSeries_Amravati_' + year,
    fileFormat: 'CSV'
  });
});
